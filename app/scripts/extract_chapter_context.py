...
- previous chapter summaries (prefers .ainovel/summaries)
...
from chapter_outline_loader import load_chapter_outline, load_chapter_plot_structure, load_chapter_execution_directive
...
def find_project_root(start_path: Path | None = None) -> Path:
    """解析真实书项目根（包含 `.ainovel/state.json` 的目录）。"""
...
def extract_chapter_outline(project_root: Path, chapter_num: int) -> str:
    ...
    return load_chapter_outline(project_root, chapter_num, max_chars=5000)


def _load_summary_file(project_root: Path, chapter_num: int) -> str:
    """Load summary section from `.ainovel/summaries/chNNNN.md`."""
    summary_path = project_root / ".ainovel" / "summaries" / f"ch{chapter_num:04d}.md"
...
def extract_state_summary(project_root: Path) -> str:
    """Extract key fields from `.ainovel/state.json`."""
    state_file = project_root / ".ainovel" / "state.json"
...
def build_chapter_context_payload(project_root: Path, chapter_num: int) -> Dict[str, Any]:
    ...
    directive = load_chapter_execution_directive(project_root, chapter_num)
    paragraph_beats = directive.get("paragraph_beats", [])
    ...
    return {
        ...
        "paragraph_beats": paragraph_beats,
