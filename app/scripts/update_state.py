...
class StateUpdater:
    ...
    def add_chapters_planned(self, volume: int, chapters_range: str):
        """增量登记本次已规划的章范围（每次按批规划用，不标记整卷完成）。

        与 mark_volume_planned 区别：同卷允许多条记录、不覆盖，
        用于 plan 每次 10 章增量登记，供 chapter_outline_loader 反查章所属卷号。
        """
        if "progress" not in self.state or not isinstance(self.state["progress"], dict):
            self.state["progress"] = {}
        chapters_planned = self.state["progress"].setdefault("chapters_planned", [])
        if not isinstance(chapters_planned, list):
            chapters_planned = []
            self.state["progress"]["chapters_planned"] = chapters_planned
        # 去重：完全相同的 (volume, chapters_range) 不重复登记
        for item in chapters_planned:
            if isinstance(item, dict) and item.get("volume") == volume and item.get("chapters_range") == chapters_range:
                print(f"⚠️  第{volume}卷第{chapters_range}章已登记，跳过")
                return
        chapters_planned.append({
            "volume": volume,
            "chapters_range": chapters_range,
            "planned_at": datetime.now().strftime("%Y-%m-%d")
        })
        print(f"📝 登记第{volume}卷已规划章范围: 第{chapters_range}章")
...
示例：
  ...
  python update_state.py \
    ...
    parser.add_argument(
        '--project-root',
        default=None,
        help='项目根目录（包含 .ainovel/state.json）。不提供时自动搜索（支持 ainovel-project/ 与父目录）。'
    )

    parser.add_argument(
        '--state-file',
        default=None,
        help='state.json 文件路径（可选）。不提供时从项目根目录自动定位为 .ainovel/state.json。'
    )
    ...
    # 增量登记已规划章范围（plan 每次 N 章用，不标记整卷完成）
    parser.add_argument(
        '--add-chapters-planned',
        action='store_true',
        help='增量登记本次已规划的章范围（配合 --volume 与 --chapters-range）'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        metavar='VOLUME',
        help='卷号（配合 --add-chapters-planned 使用）'
    )
    ...
    try:
        ...
        if args.add_chapters_planned:
            if not args.volume:
                print("❌ --add-chapters-planned 需要 --volume 参数")
                sys.exit(1)
            if not args.chapters_range:
                print("❌ --add-chapters-planned 需要 --chapters-range 参数")
                sys.exit(1)
            updater.add_chapters_planned(args.volume, args.chapters_range)
