...
功能：
1. 自动 Git 提交：每次 /ainovel-write 完成后自动 commit
...
class GitBackupManager:
    ...
    def __init__(self, project_root: str):
        ...
        if not self.git_dir.exists():
            print("⚠️  Git 未初始化，请先运行 /ainovel-init 或手动执行 git init")
            print("💡 现在自动初始化 Git...")
            self._init_git()
...
""")
    ...
    def _local_backup(self, chapter_num: int) -> bool:
        """本地备份（Git 不可用时的降级方案）"""
        backup_dir = self.project_root / ".ainovel" / "backups"
        ...
        try:
            # 备份 state.json
            state_file = self.project_root / ".ainovel" / "state.json"
    ...
    def diff(self, chapter_a: int, chapter_b: int):
        ...
        success, state_diff = self._run_git_command(
            ["diff", tag_a, tag_b, "--", ".ainovel/state.json"]
        )
