...
class EmbeddingAPIClient:
    ...
    def _build_url(self) -> str:
        ...
        兼容多种 base_url 形式：
        - 已以 ``/embeddings`` 结尾：原样返回
        - 以 ``/v\\d+`` 结尾（如 ``/v1``、``/v3``、火山引擎 ``/api/coding/v3``）：
          直接 + ``/embeddings``
        - 裸 host（如 ``https://host``）：补 ``/v1/embeddings``
        """
    ...
    async def embed(self, texts: List[str]) -> Optional[List[List[float]]]:
        ...
        # 429 限流给更大重试预算与更长退避：火山引擎 Ark 账号级 RPM 限制下，
        # 默认 3 次/1~4s 退避不足以等过限流窗口，会整批失败导致语义检索降级。
        rate_limit_retries = int(os.getenv("EMBED_RATE_LIMIT_RETRIES", "8"))
        rate_limit_base = float(os.getenv("EMBED_RATE_LIMIT_DELAY", "3.0"))
        ...
        async with self.sem:
            ...
            attempt = 0
            # 上限：普通重试 + 限流额外重试，避免无限循环
            hard_cap = max_retries + rate_limit_retries
            while attempt < hard_cap:
                try:
                    ...
                    async with session.post(
                        ...
                        # 429 限流：优先尊重 Retry-After，否则指数退避（更长基底）
                        if resp.status == 429 and attempt < hard_cap - 1:
                            retry_after = resp.headers.get("Retry-After") or resp.headers.get("retry-after")
                            if retry_after:
                                try:
                                    delay = float(retry_after)
                                except ValueError:
                                    delay = rate_limit_base * (2 ** min(attempt, 5))
                            else:
                                delay = rate_limit_base * (2 ** min(attempt, 5))
                            delay = min(delay, 60.0)
                            print(f"[WARN] Embed 429 rate-limited, retrying in {delay:.1f}s ({attempt + 1}/{hard_cap})")
                            attempt += 1
                            await asyncio.sleep(delay)
                            continue

                        # 其它可重试状态码: 500, 502, 503, 504
                        if resp.status in (500, 502, 503, 504) and attempt < max_retries - 1:
...
class RerankAPIClient:
    ...
    def _build_url(self) -> str:
        """构建请求 URL（与 EmbeddingAPIClient 同规则，处理 /v\\d+ 形 base）"""
    ...
    async def rerank(
        ...
        max_retries = getattr(self.config, 'api_max_retries', 3)
        base_delay = getattr(self.config, 'api_retry_delay', 1.0)
        # 429 限流给更大重试预算与更长退避（与 EmbeddingAPIClient.embed 一致）。
        rate_limit_retries = int(os.getenv("RERANK_RATE_LIMIT_RETRIES", "8"))
        rate_limit_base = float(os.getenv("RERANK_RATE_LIMIT_DELAY", "3.0"))
            ...
            while attempt < hard_cap:
                try:
                    ...
                    async with session.post(
                        ...
                        # 429 限流：尊重 Retry-After，否则指数退避（更长基底）
                        if resp.status == 429 and attempt < hard_cap - 1:
                            retry_after = resp.headers.get("Retry-After") or resp.headers.get("retry-after")
                            if retry_after:
                                try:
                                ...
                                delay = rate_limit_base * (2 ** min(attempt, 5))
                            delay = min(delay, 60.0)
                            print(f"[WARN] Rerank 429 rate-limited, retrying in {delay:.1f}s ({attempt + 1}/{hard_cap})")
                            attempt += 1
                            await asyncio.sleep(delay)
                            continue
