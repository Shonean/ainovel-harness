...
class ChapterCommitService:
    ...
    def build_commit(
        ...
        # 同时检查 final_audit.json 的 blocking 计数（lint/quant 等轨道的硬违规）
        final_audit_path = self.project_root / ".ainovel" / "tmp" / "final_audit.json"
        if final_audit_path.is_file():
            try:
                ...
                if isinstance(final_audit, dict):
                    summary = final_audit.get("summary", {})
                    if isinstance(summary, dict) and summary.get("blocking", 0) > 0:
                        rejected = True
            except Exception:
                pass
