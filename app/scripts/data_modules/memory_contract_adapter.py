...
包含 memory_contract.py 的类型定义（合并为一个模块）。
"""
...
class MemoryContract(Protocol):
    """记忆模块统一契约。上层消费者依赖此 Protocol，不依赖具体实现类。"""
...
class MemoryContractAdapter:
    ...
    def _commit_chapter_legacy(self, chapter: int, result: dict) -> CommitResult:
        ...
        summary_file = self.config.ainovel_dir / "summaries" / f"ch{padded}.md"
    ...
    def _commit_chapter_mainline(self, chapter: int, result: dict) -> CommitResult:
        ...
        summary_file = self.config.ainovel_dir / "summaries" / f"ch{chapter:04d}.md"
        return CommitResult(
            chapter=chapter,
    ...
    def load_context(self, chapter: int, budget_tokens: int = 4000) -> ContextPack:
        ...
        try:
            ...
            outline = load_chapter_outline(self.config.project_root, chapter, max_chars=5000)
