...
# -*- coding: utf-8 -*-
"""
ainovel 统一入口（面向 skills / agents 的稳定 CLI）

设计目标：
- 只有一个入口命令，避免到处拼 `python -m data_modules.xxx ...` 导致参数位置/引号/路径炸裂。
- 自动解析正确的 book project_root（包含 `.ainovel/state.json` 的目录）。
- 所有写入类命令在解析到 project_root 后，统一前置 `--project-root` 传给具体模块。

典型用法（推荐，不依赖 PYTHONPATH / 不要求 cd）：
  python "<SCRIPTS_DIR>/ainovel.py" preflight
  python "<SCRIPTS_DIR>/ainovel.py" where
  python "<SCRIPTS_DIR>/ainovel.py" use D:\\wk\\xiaoshuo\\凡人资本论
  python "<SCRIPTS_DIR>/ainovel.py" --project-root D:\\wk\\xiaoshuo index stats
  python "<SCRIPTS_DIR>/ainovel.py" --project-root D:\\wk\\xiaoshuo state process-chapter --chapter 100 --data @payload.json
  python "<SCRIPTS_DIR>/ainovel.py" --project-root D:\\wk\\xiaoshuo extract-context --chapter 100 --format json

也支持（不推荐，容易踩 PYTHONPATH/cd/参数顺序坑）：
  python -m data_modules.ainovel where
"""
...
# ── KnowledgeQuery（来自 knowledge_query.py） ──

class KnowledgeQuery:
    def __init__(self, project_root: Path):
        self.project_root = Path(project_root)
        self._db_path = self.project_root / ".ainovel" / "index.db"
...
def _scripts_dir() -> Path:
    # data_modules/ainovel.py -> data_modules -> scripts
...
def _project_root_diagnostic(
    ...
    if explicit_project_root:
        return (
            "未找到有效书项目根目录（需要包含 .ainovel/state.json）: "
            f"{explicit_project_root}\n"
            f"detail: {exc}"
        )
    return (
        "当前工作区还没有激活的书项目（未找到 .ainovel/state.json）。\n"
        "请先运行 ainovel init 创建项目，或运行 ainovel use <project_root> 绑定已有书项目。\n"
        f"detail: {exc}"
    )
...
def _build_preflight_report(explicit_project_root: Optional[str]) -> dict:
    ...
    skill_root = plugin_root / "skills" / "ainovel-write"
    entry_script = scripts_dir / "ainovel.py"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ainovel unified CLI")
    ...
    p_use.add_argument("project_root", help="书项目根目录（必须包含 .ainovel/state.json）")
    ...
    p_chapter_directive = sub.add_parser("chapter-directive", help="输出结构化章纲指令（含完整paragraph_beats）")
    p_chapter_directive.add_argument("--chapter", type=int, required=True, help="目标章节号")
    ...
    if tool == "chapter-directive":
        from chapter_outline_loader import load_chapter_execution_directive
        directive = load_chapter_execution_directive(project_root, args.chapter)
    ...
    if tool == "placeholder-scan":
        raise SystemExit(_run_data_module("prewrite_validator", [*forward_args, "--format", str(args.format)]))
