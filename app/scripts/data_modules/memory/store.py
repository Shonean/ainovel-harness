...
class ScratchpadManager:
    ...
    def mark_outdated_by_source_chapter(self, chapter: int) -> int:
        """把 source_chapter == chapter 的所有 active items 标为 outdated。

        用于 commit 前清场：重写章节 N 时，先把上一版 commit 写入的 N 章事实标为 outdated，
        再写入新事实，避免 RAG / load-context 搜回旧版的「剧情事实」造成自馈循环。

        Returns:
            标记为 outdated 的 item 数
        """
        if chapter <= 0:
            return 0
        marked = 0
        with self._lock:
            ...
            for bucket in BUCKET_TO_CATEGORY:
                rows: List[MemoryItem] = getattr(data, bucket)
                for i, row in enumerate(rows):
                    if row.source_chapter == chapter and row.status == "active":
                        rows[i] = MemoryItem(**{**asdict(row), "status": "outdated", "updated_at": now_iso()})
                        marked += 1
            if marked:
                self.save(data, _use_lock=False)
        return marked
