...
# ── Budget helpers (merged from budget.py) ────────────────────────────
...
class MemoryOrchestrator:
    ...
    def _filter_relevant(self, items: List[MemoryItem], chapter: int, outline: str) -> List[MemoryItem]:
        ...
        for item in items:
            ...
            if item.value and item.value[:20] in outline:
                keep.append(item)
                continue
            if item.source_chapter > 0 and item.source_chapter < chapter and chapter - item.source_chapter <= source_window:
                keep.append(item)
    ...
    def _load_recent_summaries(self, chapter: int, window: int) -> List[Dict[str, Any]]:
        ...
        summary_dir = self.config.ainovel_dir / "summaries"
