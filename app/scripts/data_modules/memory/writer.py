...
class MemoryWriter:
    ...
    def apply_commit_projection(self, commit_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 切断 Memory 自馈：先把上一版 commit 写入的 chapter-N 事实标 outdated，
        # 避免重写时 RAG / load-context 搜回旧版「剧情事实」导致新写与旧写雷同。
        # 默认跳过 ch1（ch1 没有任何 active memory 来源，不需清理）。
        if chapter > 1:
            try:
                from .store import ScratchpadManager
                marked = ScratchpadManager().mark_outdated_by_source_chapter(chapter)
                if marked:
                    import logging
                    logging.getLogger(__name__).info(
                        "memory: marked %d items outdated for chapter %d (pre-write purge)", marked, chapter,
                    )
            except Exception as e:  # 清理失败不影响 commit 主流程
                import logging
                logging.getLogger(__name__).warning("memory: pre-write purge failed for chapter %d: %s", chapter, e)
...
def purge_chapter_outdated(project_root, chapter: int) -> int:
    """重置 chapter N 在 memory_scratchpad.json 中的所有 active items（标 outdated）。

    供 chapter_reset_service 调用。Returns: 标记为 outdated 的 item 数。
    """
    if chapter <= 0:
        return 0
    return ScratchpadManager(
        DataModulesConfig.from_project_root(project_root)
    ).mark_outdated_by_source_chapter(chapter)
