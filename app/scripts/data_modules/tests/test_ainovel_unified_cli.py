...
def _load_ainovel_cli():
    _ensure_scripts_on_path()
    import data_modules.ainovel as ainovel_cli

    return ainovel_cli


def test_init_does_not_resolve_existing_project_root(monkeypatch):
    module = _load_ainovel_cli()
    ...
    monkeypatch.setenv("AINOVEL_PROJECT_ROOT", r"D:\invalid\root")
    monkeypatch.setattr(module, "_run_script", _fake_run_script)
    monkeypatch.setattr(module, "_resolve_root", _fail_resolve)
    monkeypatch.setattr(sys, "argv", ["ainovel", "init", "proj-dir", "测试书", "修仙"])
...
def test_backup_forwards_resolved_book_root_from_parent_workspace(monkeypatch, tmp_path):
    ...
    (book_root / ".ainovel" / "state.json").write_text("{}", encoding="utf-8")
    called = {}
...
def test_ainovel_story_system_forwards_with_resolved_project_root(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
...
def test_ainovel_story_system_runtime_forwards(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
...
def test_ainovel_commit_forwards(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    project_root = tmp_path / "book"
    (project_root / ".ainovel").mkdir(parents=True, exist_ok=True)
    (project_root / ".ainovel" / "state.json").write_text("{}", encoding="utf-8")
    called = {}
    ...
    monkeypatch.setattr(module, "_run_script", _fake_run_script)
    monkeypatch.setattr(sys, "argv", ["ainovel", "--project-root", str(project_root), "chapter-commit", "--chapter", "3"])
...
def test_ainovel_story_events_forwards(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    project_root = tmp_path / "book"
    (project_root / ".ainovel").mkdir(parents=True, exist_ok=True)
    (project_root / ".ainovel" / "state.json").write_text("{}", encoding="utf-8")
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["ainovel", "--project-root", str(project_root), "story-events", "--chapter", "3"],
    )
...
def test_preflight_succeeds_for_valid_project_root(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["ainovel", "--project-root", str(project_root), "preflight"])
...
def test_preflight_fails_when_required_scripts_are_missing(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["ainovel", "--project-root", str(project_root), "preflight", "--format", "json"])
...
def test_preflight_includes_story_runtime_health(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["ainovel", "--project-root", str(project_root), "preflight", "--format", "json"],
    )
...
def test_where_reports_empty_workspace_without_traceback(monkeypatch, tmp_path, capsys):
    module = _load_ainovel_cli()
    # conftest 的 tmp_path 落在仓库内 .tmp/pytest/——若仓库本身嵌在一个
    # ainovel 项目下（开发机常见情形），_resolve_root 会向上走找到
    # .ainovel/state.json 导致测试假阴。直接 mock _resolve_root 让它
    # 抛出预期的 FileNotFoundError，回到这个测试本意：验证 cmd_where
    # 在「找不到项目」分支下退出码 1、有中文提示、不暴露 traceback。
    workspace = tmp_path / "workspace"
    workspace.mkdir(parents=True, exist_ok=True)
    (workspace / ".git").mkdir(parents=True, exist_ok=True)

    monkeypatch.chdir(workspace)
    monkeypatch.delenv("AINOVEL_PROJECT_ROOT", raising=False)
    monkeypatch.delenv("CLAUDE_PROJECT_DIR", raising=False)
    monkeypatch.setenv("AINOVEL_CLAUDE_HOME", str(tmp_path / "empty-claude-home"))
    monkeypatch.setattr(sys, "argv", ["ainovel", "where"])

    def _fake_resolve(_explicit=None):
        raise FileNotFoundError(
            "Unable to locate ainovel project root. Expected `.ainovel/state.json` "
            "under the current directory, a parent directory, or `ainovel-project/`."
        )
...
def test_preflight_reports_empty_workspace_without_traceback(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.delenv("CLAUDE_PROJECT_DIR", raising=False)
    monkeypatch.setenv("AINOVEL_CLAUDE_HOME", str(tmp_path / "empty-claude-home"))
    monkeypatch.setattr(sys, "argv", ["ainovel", "preflight", "--format", "json"])
...
def test_quality_trend_report_writes_to_book_root_when_input_is_workspace_root(tmp_path, monkeypatch):
    ...
    (workspace_root / ".claude" / ".ainovel-current-project").write_text(str(book_root), encoding="utf-8")
    ...
    assert (book_root / ".ainovel" / "index.db").is_file()
    assert not (workspace_root / ".ainovel" / "index.db").exists()
...
def test_project_memory_add_pattern_escapes_quotes(tmp_path):
    ...
    project_root = (tmp_path / "book").resolve()
    (project_root / ".ainovel").mkdir(parents=True, exist_ok=True)
    (project_root / ".ainovel" / "state.json").write_text(
    ...
    memory_path = project_root / ".ainovel" / "project_memory.json"
...
def test_review_pipeline_main_creates_output_directories(tmp_path):
    ...
    metrics_out = project_root / ".ainovel" / "tmp" / "review" / "metrics.json"
    ...
    with sqlite3.connect(project_root / ".ainovel" / "index.db") as conn:
        row = conn.execute(
...
def test_ainovel_skill_flow_runs_story_contract_context_and_review_pipeline_with_stubbed_vector_model(
    ...
    def _run_ainovel(argv):
        monkeypatch.setattr(sys, "argv", ["ainovel", *argv])
        with pytest.raises(SystemExit) as exc:
            module.main()
    ...
    review_results_path = project_root / ".ainovel" / "tmp" / "review_results.json"
    ...
    metrics_out = project_root / ".ainovel" / "tmp" / "review_metrics.json"
    assert (
        _run_ainovel(
            [
                "--project-root",
...
def test_knowledge_query_entity_state_cli(monkeypatch, tmp_path, capsys):
    """ainovel knowledge query-entity-state 走 KnowledgeQuery 并以 exit 0 退出。"""
    module = _load_ainovel_cli()
    import data_modules.knowledge_query as kq_mod
    ...
    class _FakeKQ:
        ...
        def entity_state_at_chapter(self, entity, at_chapter):
            calls["entity_state"] = (entity, at_chapter)
            return {"entity": entity, "chapter": at_chapter, "state": "alive"}

    monkeypatch.setattr(module, "_resolve_root", lambda _explicit=None: book_root)
    monkeypatch.setattr(kq_mod, "KnowledgeQuery", _FakeKQ)
    monkeypatch.setattr(sys, "argv", ["ainovel", "knowledge", "query-entity-state",
                                      "--entity", "E001", "--at-chapter", "5"])

    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
    assert calls["project_root"] == book_root
    assert calls["entity_state"] == ("E001", 5)
    out = capsys.readouterr().out
    assert "entity_state_at_chapter" in out


def test_knowledge_query_relationships_cli(monkeypatch, tmp_path, capsys):
    """ainovel knowledge query-relationships 走 KnowledgeQuery 并以 exit 0 退出。"""
    module = _load_ainovel_cli()
    import data_modules.knowledge_query as kq_mod
    ...
    class _FakeKQ:
        ...
        def entity_relationships_at_chapter(self, entity, at_chapter):
            calls["rel"] = (entity, at_chapter)
            return {"entity": entity, "chapter": at_chapter, "relationships": []}

    monkeypatch.setattr(module, "_resolve_root", lambda _explicit=None: book_root)
    monkeypatch.setattr(kq_mod, "KnowledgeQuery", _FakeKQ)
    monkeypatch.setattr(sys, "argv", ["ainovel", "knowledge", "query-relationships",
                                      "--entity", "E001", "--at-chapter", "5"])

    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
    assert calls["rel"] == ("E001", 5)
    out = capsys.readouterr().out
    assert "entity_relationships_at_chapter" in out
