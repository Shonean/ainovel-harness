...
def generate_state(project_root: Path) -> None:
    ...
    (ainovel / "state.json").write_text(
        json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8"
    )


def generate_index_db(project_root: Path) -> None:
    db_path = project_root / ".ainovel" / "index.db"
    ...
    # vector db (模拟)
    vector_db_path = project_root / ".ainovel" / "vectors.db"
...
def main() -> None:
    ...
    print(f"  state.json  : {demo_root / '.ainovel' / 'state.json'}")
    print(f"  index.db    : {demo_root / '.ainovel' / 'index.db'}")
    print(f"  vectors.db  : {demo_root / '.ainovel' / 'vectors.db'}")
