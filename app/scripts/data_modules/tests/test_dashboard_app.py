...
def _create_dashboard_client(monkeypatch, project_root: Path) -> TestClient:
    ...
    # 模块隔离：API key 走用户级全局 .env（AINOVEL_CLAUDE_HOME 指向临时目录）。
    # 先清掉 os.environ 里可能残留的真实 key，再写一份测试用用户级 .env，
    # 由 create_app() 内的 load_user_env()（fresh 模块，幂等标志为 False）读入。
    for _k in (
        "EMBED_API_KEY", "RERANK_API_KEY", "ARK_API_KEY", "ANTHROPIC_API_KEY",
        "EMBED_BASE_URL", "EMBED_MODEL", "RERANK_BASE_URL", "RERANK_MODEL",
        "ARK_BASE_URL", "ARK_MODEL", "ANTHROPIC_BASE_URL", "ANTHROPIC_MODEL",
    ):
        monkeypatch.delenv(_k, raising=False)

    monkeypatch.setenv("AINOVEL_CLAUDE_HOME", str(project_root.parent))
    _user_env_dir = project_root.parent / "ainovel-write"
    _user_env_dir.mkdir(parents=True, exist_ok=True)
    (_user_env_dir / ".env").write_text(
        "\n".join(
            [
...
def _write_state(project_root: Path) -> None:
    ...
    ainovel_dir = project_root / ".ainovel"
