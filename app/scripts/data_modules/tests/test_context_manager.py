...
from data_modules.rag_adapter import QueryRouter
...
def test_context_manager_build_and_filter(temp_project):
    ...
    (temp_project.ainovel_dir / "preferences.json").write_text(json.dumps({"tone": "热血"}, ensure_ascii=False), encoding="utf-8")
    (temp_project.ainovel_dir / "project_memory.json").write_text(json.dumps({"patterns": []}, ensure_ascii=False), encoding="utf-8")
