...
# -*- coding: utf-8 -*-
"""
补全 store CLI, compactor 边界, cli_args 解析, ainovel 路由 的测试覆盖。
"""
...
def test_ainovel_cmd_where(monkeypatch, tmp_path, capsys):
...
def test_ainovel_passthrough_state(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    book_root = tmp_path / "book"
    called = {}
    ...
    monkeypatch.setattr(sys, "argv", ["ainovel", "state", "get-progress"])
...
def test_ainovel_passthrough_memory(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    book_root = tmp_path / "book"
    called = {}
    ...
    monkeypatch.setattr(sys, "argv", ["ainovel", "memory", "stats"])
...
def test_ainovel_passthrough_style(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    book_root = tmp_path / "book"
    called = {}
    monkeypatch.setattr(module, "_resolve_root", lambda _=None: book_root)
    monkeypatch.setattr(module, "_run_data_module", lambda m, a: (called.update(mod=m), 0)[1])
    monkeypatch.setattr(sys, "argv", ["ainovel", "style", "list"])
    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
...
def test_ainovel_passthrough_entity(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    called = {}
    monkeypatch.setattr(module, "_resolve_root", lambda _=None: tmp_path)
    monkeypatch.setattr(module, "_run_data_module", lambda m, a: (called.update(mod=m), 0)[1])
    monkeypatch.setattr(sys, "argv", ["ainovel", "entity", "process"])
    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
...
def test_ainovel_passthrough_context(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    called = {}
    monkeypatch.setattr(module, "_resolve_root", lambda _=None: tmp_path)
    monkeypatch.setattr(module, "_run_data_module", lambda m, a: (called.update(mod=m), 0)[1])
    monkeypatch.setattr(sys, "argv", ["ainovel", "context", "build"])
    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
...
def test_ainovel_passthrough_migrate(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    called = {}
    monkeypatch.setattr(module, "_resolve_root", lambda _=None: tmp_path)
    monkeypatch.setattr(module, "_run_data_module", lambda m, a: (called.update(mod=m), 0)[1])
    monkeypatch.setattr(sys, "argv", ["ainovel", "migrate"])
    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
...
def test_ainovel_passthrough_status_script(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    called = {}
    monkeypatch.setattr(module, "_resolve_root", lambda _=None: tmp_path)
    monkeypatch.setattr(module, "_run_script", lambda s, a: (called.update(script=s), 0)[1])
    monkeypatch.setattr(sys, "argv", ["ainovel", "status"])
    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
...
def test_ainovel_passthrough_update_state_script(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    called = {}
    monkeypatch.setattr(module, "_resolve_root", lambda _=None: tmp_path)
    monkeypatch.setattr(module, "_run_script", lambda s, a: (called.update(script=s), 0)[1])
    monkeypatch.setattr(sys, "argv", ["ainovel", "update-state", "add-review"])
    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
...
def test_ainovel_passthrough_backup_script(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    called = {}
    monkeypatch.setattr(module, "_resolve_root", lambda _=None: tmp_path)
    monkeypatch.setattr(module, "_run_script", lambda s, a: (called.update(script=s), 0)[1])
    monkeypatch.setattr(sys, "argv", ["ainovel", "backup"])
    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
...
def test_ainovel_passthrough_archive_script(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    called = {}
    monkeypatch.setattr(module, "_resolve_root", lambda _=None: tmp_path)
    monkeypatch.setattr(module, "_run_script", lambda s, a: (called.update(script=s), 0)[1])
    monkeypatch.setattr(sys, "argv", ["ainovel", "archive"])
    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
...
def test_ainovel_remainder_strips_leading_double_dash(monkeypatch, tmp_path):
    module = _load_ainovel_cli()
    called = {}
    monkeypatch.setattr(module, "_resolve_root", lambda _=None: tmp_path)
    monkeypatch.setattr(module, "_run_data_module", lambda m, a: (called.update(argv=list(a)), 0)[1])
    monkeypatch.setattr(sys, "argv", ["ainovel", "index", "--", "get-core-entities"])
    with pytest.raises(SystemExit) as exc:
        module.main()
    assert int(exc.value.code or 0) == 0
...
def test_ainovel_cmd_use(monkeypatch, tmp_path, capsys):
    module = _load_ainovel_cli()
    book_root = tmp_path / "book"
    ...
    monkeypatch.setattr(sys, "argv", ["ainovel", "use", str(book_root)])
...
def test_ainovel_cmd_use_with_workspace_root(monkeypatch, tmp_path, capsys):
    module = _load_ainovel_cli()
    book_root = tmp_path / "book"
    ...
    monkeypatch.setattr(sys, "argv", ["ainovel", "use", str(book_root), "--workspace-root", str(workspace_root)])
...
def test_ainovel_resolve_root_fallback(monkeypatch):
    module = _load_ainovel_cli()
