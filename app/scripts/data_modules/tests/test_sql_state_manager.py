...
def test_sql_state_manager_entity_and_alias(temp_project):
    ...
    fetched = manager.get_entity("linyue")
    assert "炎帝" in fetched["aliases"]

    by_type = manager.get_entities_by_type("角色")
    assert any(e["id"] == "linyue" for e in by_type)

    core = manager.get_core_entities()
    assert any(e["id"] == "linyue" for e in core)

    protagonist = manager.get_protagonist()
    assert protagonist["id"] == "linyue"

    resolved = manager.resolve_alias("炎帝")
    assert any(r["id"] == "linyue" for r in resolved)

    assert manager.update_entity_current("linyue", {"realm": "斗王"}) is True
...
def test_sql_state_manager_state_changes_and_relationships(temp_project):
    ...
    manager.upsert_entity(
        EntityData(id="linyue", type="角色", name="林越", current={})
    )
    ...
    rels = manager.get_entity_relationships("linyue", direction="from")
    assert len(rels) == 1
    between = manager.get_relationship_between("linyue", "laoyaoshi")
...
def test_sql_state_manager_process_chapter_entities_and_exports(temp_project):
    ...
    stats = manager.process_chapter_entities(
        chapter=10,
        entities_appeared=[{"id": "linyue", "mentions": ["林越"], "confidence": 0.9}],
        entities_new=[
            {"suggested_id": "laoyaoshi", "name": "药师", "type": "角色", "tier": "重要"}
        ],
        state_changes=[
            {"entity_id": "laoyaoshi", "field": "status", "old": "", "new": "出场", "reason": "登场"}
        ],
        relationships_new=[
    ...
    rel_events = manager._index_manager.get_relationship_events("linyue", direction="both")
...
def test_sql_state_manager_existing_entity_updates_and_stats(temp_project):
    manager = SQLStateManager(temp_project)
    manager.upsert_entity(
        EntityData(id="linyue", type="角色", name="林越", current={"hp": 5})
    )
    ...
    stats = manager.process_chapter_entities(
        ...
        entities_new=[],
        state_changes=[
            {"entity_id": "linyue", "field": "hp", "old": 5, "new": 0, "reason": "受伤"}
        ],
        relationships_new=[
            {"from_entity": "linyue", "to_entity": "laoyaoshi", "type": "师徒", "description": "收徒"}
        ],
    )
    ...
    rels = manager.get_entity_relationships("laoyaoshi", direction="to")
    assert rels
    ...
    assert exported["角色"]["linyue"]["canonical_name"] == "林越"


def test_sql_state_manager_process_chapter_skips_and_existing(temp_project):
    manager = SQLStateManager(temp_project)
    manager.upsert_entity(EntityData(id="linyue", type="角色", name="林越"))

    stats = manager.process_chapter_entities(
        chapter=1,
        entities_appeared=[{"mentions": ["无ID"]}, {"id": "linyue", "mentions": ["林越"]}],
        entities_new=[{"name": "无ID"}, {"suggested_id": "linyue", "name": "林越"}],
        state_changes=[{"field": "realm"}, {"entity_id": "linyue", "field": "hp", "old": 1, "new": 1}],
        relationships_new=[{"from": "linyue", "to": ""}],
    )
    assert stats["entities_updated"] >= 1
...
def test_sql_state_manager_export_protagonist_and_cli(temp_project, monkeypatch, capsys):
    ...
    manager.upsert_entity(
        EntityData(id="linyue", type="角色", name="林越", is_protagonist=True)
    )
    exported = manager.export_to_entities_v3_format()
    assert exported["角色"]["linyue"]["is_protagonist"] is True

    out = run_cli(["sql_state_manager", "--project-root", str(temp_project.project_root), "get-protagonist"])
    assert out["status"] == "success"
    assert out["data"].get("canonical_name") == "林越"
