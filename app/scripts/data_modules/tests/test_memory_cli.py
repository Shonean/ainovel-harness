...
def _make_project(tmp_path: Path):
    ...
    (ainovel_dir / "state.json").write_text("{}", encoding="utf-8")
    (ainovel_dir / "summaries").mkdir(exist_ok=True)
    return tmp_path
...
def test_read_summary_exists(tmp_path, capsys):
    ...
    project = _make_project(tmp_path)
    (project / ".ainovel" / "summaries" / "ch0005.md").write_text("第5章摘要", encoding="utf-8")
