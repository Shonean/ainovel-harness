...
def _ensure_project_state(config):
    config.ensure_dirs()
    state_path = config.ainovel_dir / "state.json"
...
class TestStyleSampler:
    ...
    def test_user_cold_start_anchors_loaded_from_dir(self, temp_project):
        """用户冷启动锚点应从 .ainovel/style_anchors_user/ 读取"""
        sampler = StyleSampler(temp_project)

        anchors_dir = sampler.config.ainovel_dir / "style_anchors_user"
        anchors_dir.mkdir(parents=True, exist_ok=True)

        anchor_file = anchors_dir / "01-紧张恐怖.md"
        anchor_file.write_text(
            ...
            "\n"
            "门外没有声音了。\n\n"
            "我把耳朵贴在木板上。心跳盖过了所有动静——这本身就该让我心安，但没有。\n",
            encoding="utf-8",
        )

        anchors = sampler.get_user_cold_start_anchors(scene_type="紧张")
        assert len(anchors) == 1
        assert anchors[0].scene_type == "紧张"
        assert anchors[0].score >= 0.95
        assert "门外没有声音了" in anchors[0].content
        assert "短句" in anchors[0].tags

    def test_select_samples_falls_back_to_user_anchors_when_db_empty(self, temp_project):
        """db 为空时 select 应回退到用户冷启动锚点"""
        sampler = StyleSampler(temp_project)

        anchors_dir = sampler.config.ainovel_dir / "style_anchors_user"
        anchors_dir.mkdir(parents=True, exist_ok=True)
        (anchors_dir / "battle_anchor.md").write_text(
            "---\nscene_type: 战斗\nscore: 0.97\n---\n\n他抬手挡了一下。骨头响。\n",
            encoding="utf-8",
        )

        # db 没有任何 sample
        samples = sampler.select_samples_for_chapter("本章是一场战斗")
        assert len(samples) >= 1
        assert any(s.id.startswith("user_anchor_") for s in samples)

    def test_user_anchor_without_frontmatter_is_skipped(self, temp_project):
        """无 frontmatter 的锚点文件应被跳过（避免吃 README/笔记）"""
        sampler = StyleSampler(temp_project)
        anchors_dir = sampler.config.ainovel_dir / "style_anchors_user"
        anchors_dir.mkdir(parents=True, exist_ok=True)
        (anchors_dir / "raw.md").write_text("一段没有 frontmatter 的范本文本。", encoding="utf-8")
        (anchors_dir / "README.md").write_text("# 这是 README\n说明文档", encoding="utf-8")

        anchors = sampler.get_user_cold_start_anchors()
        # 两个文件都应被跳过：raw.md 因无 frontmatter，README.md 因文件名
        assert len(anchors) == 0

    def test_revoked_samples_are_filtered(self, temp_project):
        """unfinalize 后标 revoked 的样本不应被检索到"""
        import sqlite3

        sampler = StyleSampler(temp_project)
        sampler.add_sample(StyleSample(
            id="ch5_keep", chapter=5, scene_type="紧张",
            content="保留的样本", score=0.9, tags=[],
        ))
        sampler.add_sample(StyleSample(
            id="ch5_revoke", chapter=5, scene_type="紧张",
            content="被撤销的样本", score=0.95, tags=[],
        ))

        # 模拟 unfinalize 操作：加 revoked 列并标记一行
        db_path = sampler.config.ainovel_dir / "style_samples.db"
        with sqlite3.connect(str(db_path)) as conn:
            conn.execute("ALTER TABLE samples ADD COLUMN revoked INTEGER DEFAULT 0")
            conn.execute("UPDATE samples SET revoked=1 WHERE id='ch5_revoke'")
            conn.commit()

        results = sampler.get_samples_by_type("紧张", limit=10)
        ids = [s.id for s in results]
        assert "ch5_keep" in ids
        assert "ch5_revoke" not in ids

        # get_best_samples 也要过滤
        best = sampler.get_best_samples(limit=10)
        best_ids = [s.id for s in best]
        assert "ch5_keep" in best_ids
        assert "ch5_revoke" not in best_ids
