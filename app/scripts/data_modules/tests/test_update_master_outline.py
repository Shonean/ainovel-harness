...
def test_master_outline_sync_does_not_create_next_volume_detail_files(tmp_path):
    ...
    assert not (tmp_path / "大纲" / "第2卷-卷摘要.md").exists()
...
def test_ainovel_master_outline_sync_cli_forwards_project_root(monkeypatch, tmp_path):
    _ensure_scripts_on_path()
    ...
    monkeypatch.setattr(ainovel_cli, "_resolve_root", _fake_resolve)
    monkeypatch.setattr(ainovel_cli, "_run_script", _fake_run_script)
    monkeypatch.setattr(
        sys,
        "argv",
        [
