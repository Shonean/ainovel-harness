...
def test_resolve_project_root_stops_at_git_root(tmp_path):
    ...
    (outside_project / ".ainovel").mkdir(parents=True, exist_ok=True)
    (outside_project / ".ainovel" / "state.json").write_text("{}", encoding="utf-8")
...
def test_resolve_project_root_finds_default_subdir_within_git_root(tmp_path):
    ...
    default_project = repo_root / "ainovel-project"
    (default_project / ".ainovel").mkdir(parents=True, exist_ok=True)
    (default_project / ".ainovel" / "state.json").write_text("{}", encoding="utf-8")
...
def test_resolve_project_root_ignores_stale_pointer_and_fallbacks(tmp_path):
    ...
    (workspace / ".claude").mkdir(parents=True, exist_ok=True)
    # stale pointer
    (workspace / ".claude" / ".ainovel-current-project").write_text(
        str(workspace / "missing-project"), encoding="utf-8"
    )

    default_project = workspace / "ainovel-project"
    (default_project / ".ainovel").mkdir(parents=True, exist_ok=True)
    (default_project / ".ainovel" / "state.json").write_text("{}", encoding="utf-8")
