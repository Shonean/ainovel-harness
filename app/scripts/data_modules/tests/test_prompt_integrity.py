...
# ainovel.py 注册的子命令（从 add_parser 提取）
...
def _extract_cli_subcommands(text: str) -> list[str]:
    """从 prompt 中提取 ainovel.py 调用的子命令。"""
    cmds = set()
    for m in re.finditer(r'ainovel\.py["\s]+--project-root\s+[^\s]+\s+([a-z][\w-]*)', text):
        cmd = m.group(1)
        cmds.add(cmd)
    return sorted(cmds)
...
def test_cli_commands_valid(prompt_file: Path):
    """prompt 中的 ainovel.py 子命令都必须在 CLI 注册表中。"""
    text = _read_text(prompt_file)
    cmds = _extract_cli_subcommands(text)
    # 排除已知例外（如 ainovel-review 的 workflow 命令待重构）
...
def test_review_schema_consistency():
    """reviewer.md 输出格式中的字段必须与 ReviewIssue（review_pipeline.py）定义匹配。"""
    ...
    # 从 review_pipeline.py 提取 ReviewIssue 字段（合并自 review_schema.py）
    schema_path = SCRIPTS_DIR / "review_pipeline.py"
    ...
    assert schema_fields, "无法从 review_pipeline.py 提取字段"
    extra = issue_fields_in_prompt - schema_fields
    assert not extra, f"reviewer.md 中有字段不在 ReviewIssue 定义中: {extra}"
...
def test_ainovel_review_skill_uses_unified_reviewer_pipeline():
    """ainovel-review 必须与 ainovel-write 使用同一套 reviewer + review-pipeline 链路。"""
    skill_text = _read_text(SKILLS_DIR / "ainovel-review" / "SKILL.md")

    assert "`reviewer`" in skill_text
    assert "Agent(" in skill_text
    assert 'subagent_type: "ainovel-write:reviewer"' in skill_text
    assert "review-pipeline" in skill_text
    assert ".ainovel/tmp/review_results.json" in skill_text
    assert ".ainovel/tmp/review_metrics.json" in skill_text
...
def test_ainovel_write_skill_uses_explicit_agent_invocation_templates():
    """ainovel-write 的关键 subagent 必须用显式 Agent(subagent_type=...) 调用模板。"""
    text = _read_text(SKILLS_DIR / "ainovel-write" / "SKILL.md")
    fm = _extract_frontmatter(text)

    assert "Agent" in fm.get("allowed-tools", "")
    for subagent in ("context-agent", "reviewer", "data-agent"):
        assert f'subagent_type: "ainovel-write:{subagent}"' in text
...
def test_story_system_runtime_contract_commands_exist():
    text = (SKILLS_DIR / "ainovel-write" / "SKILL.md").read_text(encoding="utf-8")
    assert "story-system" in text
    assert "--emit-runtime-contracts" in text


def test_ainovel_write_skill_uses_chapter_commit_as_step5_mainline():
    text = (SKILLS_DIR / "ainovel-write" / "SKILL.md").read_text(encoding="utf-8")
...
def test_ainovel_write_skill_uses_project_root_backup_not_bare_git_add():
    text = (SKILLS_DIR / "ainovel-write" / "SKILL.md").read_text(encoding="utf-8")
    assert "ainovel.py" in text
    assert "--project-root \"${PROJECT_ROOT}\" backup" in text
    assert "git add ." not in text


def test_ainovel_query_skill_prefers_story_system_and_memory_contract():
    text = (SKILLS_DIR / "ainovel-query" / "SKILL.md").read_text(encoding="utf-8")
    assert "memory-contract load-context" in text
    assert ".story-system/" in text
    assert 'cat "$PROJECT_ROOT/.ainovel/state.json"' not in text
...
def test_ainovel_write_data_agent_prompt_requires_extraction_schema():
    text = (SKILLS_DIR / "ainovel-write" / "SKILL.md").read_text(encoding="utf-8")
    assert "ainovel-write:data-agent" in text
...
def test_dashboard_and_plan_skills_surface_story_runtime_mainline():
    dashboard_text = (SKILLS_DIR / "ainovel-dashboard" / "SKILL.md").read_text(encoding="utf-8")
    plan_text = (SKILLS_DIR / "ainovel-plan" / "SKILL.md").read_text(encoding="utf-8")
    assert "story-runtime/health" in dashboard_text
    assert ".story-system/" in plan_text


def test_ainovel_write_skill_routes_step2_through_writing_brief():
    text = (SKILLS_DIR / "ainovel-write" / "SKILL.md").read_text(encoding="utf-8")
    assert "写作任务书" in text
...
def test_context_agent_and_write_skill_form_isolated_write_chain():
    ...
    skill_text = (SKILLS_DIR / "ainovel-write" / "SKILL.md").read_text(encoding="utf-8")
...
def test_no_direct_state_writes_in_write_skill():
    """ainovel-write SKILL.md 中不应有 set-chapter-status 调用。"""
    text = (SKILLS_DIR / "ainovel-write" / "SKILL.md").read_text(encoding="utf-8")
    assert "state set-chapter-status" not in text, (
        "ainovel-write 中不应直接调用 state set-chapter-status，"
...
def test_deconstruction_agent_preserves_init_handoff_and_boundaries():
    ...
    assert ".ainovel/tmp/reference_analyses/<safe-title>/" not in text
...
def test_ainovel_init_deconstruction_wiring_keeps_confirmation_gate():
    """init may consume only confirmed, transformed reference patterns."""
    text = _read_text(SKILLS_DIR / "ainovel-init" / "SKILL.md")

    assert 'subagent_type: "ainovel-write:deconstruction-agent"' in text
