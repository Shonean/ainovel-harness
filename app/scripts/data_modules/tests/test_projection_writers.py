...
from data_modules.projections import (
...
def test_state_projection_writer_handles_rejected_commit(tmp_path):
    ...
    (tmp_path / ".ainovel" / "state.json").write_text("{}", encoding="utf-8")
...
def test_reapplying_accepted_chapter_commit_does_not_double_count_words(tmp_path):
    ...
    first_state = json.loads((tmp_path / ".ainovel" / "state.json").read_text(encoding="utf-8"))

    writer.apply(payload)
    second_state = json.loads((tmp_path / ".ainovel" / "state.json").read_text(encoding="utf-8"))
...
def test_summary_projection_writer_writes_summary_markdown(tmp_path):
    ...
    summary_path = tmp_path / ".ainovel" / "summaries" / "ch0003.md"
    assert result["applied"] is True
