...
def test_extract_state_summary_accepts_dominant_key(tmp_path):
    ...
    (ainovel_dir / "state.json").write_text(json.dumps(state, ensure_ascii=False), encoding="utf-8")
...
def test_build_chapter_context_payload_includes_contract_sections(tmp_path):
    ...
    (cfg.ainovel_dir / "state.json").write_text(json.dumps(state, ensure_ascii=False), encoding="utf-8")
