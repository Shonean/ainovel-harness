...
目标：
- 生成可运行的项目结构（ainovel-project）
- 创建/更新 .ainovel/state.json（初始化配置与兼容读模型）
- 生成基础设定集与大纲模板文件（供 /ainovel-plan 与 /ainovel-write 使用）

说明：
- 该脚本是命令 /ainovel-init 的“唯一允许的文件生成入口”（与命令文档保持一致）。
...
def _ensure_state_schema(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Phase 6: 通道配置（默认使用 default 通道，行为与旧版相同）
    state.setdefault(
        "channel",
        {
            "profile": "default",
...
def _build_master_outline(target_chapters: int, *, chapters_per_volume: int = 50) -> str:
    ...
    lines: list[str] = [
        "# 总纲",
        "",
        "> 本文件为“总纲骨架”，用于 /ainovel-plan 细化为卷大纲与章纲。",
        "",
        "## 卷结构",
        "",
    ]
...
def init_project(
    ...
    *,
    ...
    # state.json（创建或增量补齐）
    state_path = project_path / ".ainovel" / "state.json"
    ...
    _write_text_if_missing(
        ...
        "\n".join(
            [
                "# AInovel Harness 书级配置示例（一般留空即可）",
                "#",
                "# 【API key 放哪】用户级全局，所有书共享一份：",
                "#   ~/.claude/ainovel-write/.env",
                "# 系统通过 load_user_env() 只读该用户级 .env，不读 CWD、不读书目录 .env，",
                "# 杜绝跨书串台。书目录不放敏感 key；本文件仅用于书级非敏感配置（通常无需填写）。",
                "# 真实 key 请填到 ~/.claude/ainovel-write/.env，勿提交到版本库。",
                "",
            ]
        )
        + "\n",
...
# Don't ignore .ainovel (we need to track state.json)
# But ignore cache files
.ainovel/context_cache.json
.ainovel/*.lock
.ainovel/*.bak
""",
                        encoding="utf-8",
...
def main() -> None:
    ...
    parser.add_argument("project_dir", help="项目目录（建议 ./ainovel-project）")
