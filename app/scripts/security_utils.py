...
def create_secure_directory(path: str, mode: int = 0o700) -> Path:
    ...
    示例:
        >>> create_secure_directory('.ainovel')
        PosixPath('.ainovel')  # drwx------ (700)
...
def atomic_write_json(
    ...
    *,
    ...
    示例:
        >>> atomic_write_json('.ainovel/state.json', {'progress': {'chapter': 10}})
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if os.environ.get("AINOVEL_TEST_RELAX_ATOMIC_REPLACE") != "1":
                    raise
...
def read_json_safe(
    ...
    示例:
        >>> state = read_json_safe('.ainovel/state.json', {})
    """
    file_path = Path(file_path)
    if default is None:
        default = {}
...
def restore_from_backup(file_path: Union[str, Path]) -> bool:
    ...
    示例:
        >>> restore_from_backup('.ainovel/state.json')
        True
    """
    file_path = Path(file_path)
    backup_path = file_path.with_suffix(file_path.suffix + '.bak')
