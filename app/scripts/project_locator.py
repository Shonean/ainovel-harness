#!/usr/bin/env python3
"""
Project location helpers for ainovel-write scripts.

Problem this solves:
- Many scripts assumed CWD is the project root and used relative paths like `.ainovel/state.json`.
- In this repo, commands/scripts are often invoked from the repo root, while the actual project lives
  in a subdirectory (default: `ainovel-project/`).
...
DEFAULT_PROJECT_DIR_NAMES: tuple[str, ...] = ("ainovel-project",)
CURRENT_PROJECT_POINTER_REL: Path = Path(".claude") / ".ainovel-current-project"

# 用户级全局映射（当 skills/agents 安装在 ~/.claude 时，项目目录可能在任意盘符）
# 该文件用于在“空上下文 + CWD 不在项目内”的情况下仍能定位到正确 project_root。
GLOBAL_REGISTRY_REL: Path = Path("ainovel-write") / "workspaces.json"

# Claude Code 常见环境变量（存在时优先作为“工作区根目录”提示）
ENV_CLAUDE_PROJECT_DIR = "CLAUDE_PROJECT_DIR"
ENV_CLAUDE_HOME = "CLAUDE_HOME"
ENV_AINOVEL_CLAUDE_HOME = "AINOVEL_CLAUDE_HOME"
...
def _get_user_claude_root() -> Path:
    raw = os.environ.get(ENV_AINOVEL_CLAUDE_HOME) or os.environ.get(ENV_CLAUDE_HOME)
    if raw:
        try:
...
def update_global_registry_current_project(
    *,
    ...
    if not _is_project_root(root):
        raise FileNotFoundError(f"Not a ainovel project root (missing .ainovel/state.json): {root}")
...
def _find_workspace_root_with_claude(start: Path, *, stop_at: Optional[Path] = None) -> Optional[Path]:
    """Find nearest ancestor containing `.claude/`.

    与 resolve_project_root 的纪律一致：当提供了 stop_at（通常为 git root）时，
    向上搜索不超过该边界。这避免在“项目位于某个 git 仓库内部”时，搜索越界爬到
    仓库之外、与本项目无关的祖先 `.claude/`（例如测试沙箱嵌在真实工作区下时
    误写真实工作区指针）。stop_at 本身仍参与搜索（git root 自身的 .claude 合法）。
    """
...
def write_current_project_pointer(project_root: Path, *, workspace_root: Optional[Path] = None) -> Optional[Path]:
    ...
    ws_root = Path(workspace_root).expanduser().resolve() if workspace_root else _find_workspace_root_with_claude(
        root, stop_at=_find_git_root(root)
    )
    if ws_root is None:
        ws_root = _find_workspace_root_with_claude(
            Path.cwd().resolve(), stop_at=_find_git_root(Path.cwd().resolve())
        )
    if ws_root is None:
...
def resolve_project_root(explicit_project_root: Optional[str] = None, *, cwd: Optional[Path] = None) -> Path:
    """
    Resolve the ainovel project root directory (the directory containing `.ainovel/state.json`).

    Resolution order:
    1) explicit_project_root (if provided)
    2) env var AINOVEL_PROJECT_ROOT (if set)
    3) Search from cwd and parents, including common subdir `ainovel-project/`
    ...
    if explicit_project_root:
        ...
        # 兼容：显式传入“工作区根目录”（含 `.claude/.ainovel-current-project` 指针）
    ...
    env_root = os.environ.get("AINOVEL_PROJECT_ROOT")
    if env_root:
        root = normalize_windows_path(env_root).expanduser().resolve()
        if _is_project_root(root):
            return root
        raise FileNotFoundError(f"AINOVEL_PROJECT_ROOT is set but invalid (missing .ainovel/state.json): {root}")
    ...
    raise FileNotFoundError(
        "Unable to locate ainovel project root. Expected `.ainovel/state.json` under the current directory, "
        "a parent directory, or `ainovel-project/`. Run /ainovel-init first or pass --project-root / set "
        "AINOVEL_PROJECT_ROOT."
    )
...
def resolve_state_file(
    ...
    *,
    ...
    """
    Resolve `.ainovel/state.json` path.
    ...
    return root / ".ainovel" / "state.json"
