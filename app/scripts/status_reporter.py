...
输出格式：
  - Markdown 报告（.ainovel/health_report.md）
  - 包含 Mermaid 图表（角色关系图、爽点热力图）

使用方式：
  # 生成完整健康报告
  python status_reporter.py --output .ainovel/health_report.md
...
class StatusReporter:
    ...
    def __init__(self, project_root: str):
        ...
        self.state_file = self.project_root / ".ainovel/state.json"
...
示例：
  ...
  python status_reporter.py --focus pacing
    ...
    parser.add_argument('--output', default='.ainovel/health_report.md',
                       help='输出文件路径')
    ...
    try:
        ...
        print(f"❌ 无法定位项目根目录（需要包含 .ainovel/state.json）: {exc}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.output == '.ainovel/health_report.md' and project_root != '.':
        output_file = Path(project_root) / '.ainovel' / 'health_report.md'
