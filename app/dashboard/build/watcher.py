...
监控 PROJECT_ROOT/.ainovel/ 与 .story-system/ 的关键文件写事件，
...
class _AInovelFileHandler(FileSystemEventHandler):
    """关注 .ainovel/ 关键文件与 .story-system/ JSON 变更。"""
    ...
    def __init__(
        ...
        notify_callback,
        *,
        watch_ainovel_dir: Path | None,
        watch_story_system_dir: Path | None,
    ):
        super().__init__()
        self._notify = notify_callback
        self._watch_ainovel_dir = Path(watch_ainovel_dir).resolve() if watch_ainovel_dir else None
    ...
    def _should_notify(self, path: Path) -> bool:
        if _is_relative_to(path, self._watch_ainovel_dir):
...
class FileWatcher:
    ...
    def start(
        ...
        *,
        ...
        """启动 watchdog observer，同时监听 .ainovel 与 .story-system。"""
        self.stop()
        self._loop = loop
        handler = _AInovelFileHandler(
            self._on_change,
            watch_ainovel_dir=watch_ainovel_dir,
            watch_story_system_dir=watch_story_system_dir,
        )
        self._observer = Observer()
        has_watch_target = False
        if watch_ainovel_dir and Path(watch_ainovel_dir).is_dir():
            self._observer.schedule(handler, str(watch_ainovel_dir), recursive=False)
    ...
    def stop(self):
        """通知所有订阅者服务器即将关闭，然后停止 watchdog。"""
        # 向所有 SSE 订阅者发送关闭通知
        msg = json.dumps({"phase": "server_shutdown", "ts": time.time()})
        for q in self._subscribers:
            try:
                q.put_nowait(msg)
            except asyncio.QueueFull:
