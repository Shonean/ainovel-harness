...
python -X utf8 "${SCRIPTS_DIR}/ainovel.py" --project-root "{project_root}" index recent-appearances --limit 20
python -X utf8 "${SCRIPTS_DIR}/ainovel.py" --project-root "{project_root}" index get-aliases --entity "{entity_id}"
python -X utf8 "${SCRIPTS_DIR}/ainovel.py" --project-root "{project_root}" index get-by-alias --alias "{alias}"

python -X utf8 "${SCRIPTS_DIR}/ainovel.py" --project-root "{project_root}" chapter-commit \
  --chapter {chapter} \
  --review-result "{project_root}/.ainovel/tmp/review_results.json" \
  --fulfillment-result "{project_root}/.ainovel/tmp/fulfillment_result.json" \
  --disambiguation-result "{project_root}/.ainovel/tmp/disambiguation_result.json" \
  --extraction-result "{project_root}/.ainovel/tmp/extraction_result.json"
```
...
产出三份 JSON 到 `.ainovel/tmp/`，**三份必须各自用 Write/Bash 写入对应文件路径，缺一不可，不得只在回复里口述内容而不落盘**。常见错误：写完 fulfillment_result 就停下口述摘要——必须继续把 disambiguation_result 和 extraction_result 也写入磁盘，三份齐全才算完成。
...
- `extraction_result.json`：必须包含 `accepted_events`、`state_deltas`、`entity_deltas`、`entities_appeared`、`scenes`、`summary_text`；能判断主导情节线时写 `dominant_strand`。`summary_text` 必须写进该文件的字段，不要作为回复正文口述输出。
