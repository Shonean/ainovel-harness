...
```bash
# 查询角色当前状态
python -X utf8 "${SCRIPTS_DIR}/ainovel.py" --project-root "${PROJECT_ROOT}" state get-entity --id "{entity_id}"

# 查询最近状态变更
python -X utf8 "${SCRIPTS_DIR}/ainovel.py" --project-root "${PROJECT_ROOT}" index get-recent-state-changes --limit 20
```
...
### 6.6 约束合规性（category: constraint）（Phase 2）

按照三大定律检查本章：

**6.6.1 大纲一致性**
- 本章情节是否超出章纲范围（CBN/CPNs/CEN 节点）？
- 新增的内容是否有章纲依据或合理衔接？
- 是否有节点被遗漏或跳过的现象？
- severity: 违反节点 `high`，多处违反或多个节点缺失 `critical`

**6.6.2 设定一致性**
- 角色使用的能力/物品/关系是否已通过已有文件或前文记录建立？
- 是否有角色使用了未获得/未升级/未铺垫的能力？
- 角色实力是否在已有记录范围内（index.db / 设定集 / 前文）？
- severity: 越级使用 `critical`，边界模糊 `high`，合理范围内 `low`

**6.6.3 实体可识别性**
- 任何新角色/地点/物品/组织是否有明确名称和足够描写？
- 是否有「一个神秘人」「某个地方」「一件宝物」等模糊指代未在下文澄清？
- 重要新实体的描写是否足以让 data-agent 自动提取？
- severity: 关键实体不可识别 `high`，次要实体模糊 `medium`
  ...
  "issues": [
    {
      ...
      "category": "continuity | setting | character | timeline | ai_flavor | logic | constraint | pacing | other",
      "location": "第N段 或 具体引用",
      "description": "问题描述",
...
`blocking_count` 必填，等于 issues 中 `blocking: true` 的条目数（无阻断时为 0）。这不是评分，是阻断问题计数，供 commit 判定 accepted/rejected。
