# Prompt Harness v2.1 独立 Web 应用（旧文档）

> 本文档描述早于 promptopt 内核的独立 Web 应用（server.py/optimizer.py + React 前端，8765 dashboard 主入口的子系统）。promptopt 的英文说明见仓库根 README.md。
...
5. 打开浏览器访问：`http://127.0.0.1:8765/prompt-harness/`（**推荐，主系统入口**）。
   独立调试端口：`http://127.0.0.1:8777`（deprecated）。
...
│       └── index.css       # 样式
├── corpus/                 # 语料目录
├── data/                   # 经验库 SQLite + FAISS + 阅读位置
├── output/                 # 蒸馏产物输出
├── tests/                  # 测试
├── run.py                  # 一键启动
├── 启动.bat                # Windows 双击启动
└── DOCS.md                 # 🆕 完整系统说明书（含所有算法详解与未解决问题）
```

> 📖 **完整文档见 [DOCS.md](DOCS.md)** — 涵盖所有核心算法原理、评分体系、热启动机制、参数搜索策略、以及尚未解决的困难。
