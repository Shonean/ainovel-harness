...
_REDUNDANCY_NAMES = ("陈迹", "陈硕", "王慧玲", "袍哥", "二刀", "老刘")
...
1. 对白装饰三件套：每个对白轮前后都配"微动作+神态/感官词+副词"（例：「老刘开口，声音裹着深秋夜里的凉意」「陈迹抬了抬眼，指尖搭在桌沿轻轻敲击」「回答得干净利落」）。原文对白密集连续时，这些是额外注水，尤其要多报。
...
12. 剧情事实虚构（对照原文报）：正文出现了原文没有的人物身份/职业/背景/事件（如原文陈迹是被二叔二婶送进青山精神病院的学生，正文却写「跑去医院当全职医生」）——这类脑补必须报。
...
15. 说话动词机械化（对照原文报）：多轮对白说话动词被统一成「开口说」「说道」「说」，而原文对应轮次各有神态动词（王慧玲阴阳怪气道/陈硕得意洋洋/王慧玲喜滋滋挽住陈硕胳膊/道了一声晦气）——把原文丰富的说话方式拍平成占位词时必须报。
16. 内容重复叙述（对照原文报）：同一事实/闪回/内心独白在生成文内相隔较远重复出现（借钱往事说两遍、大嫂院子烧烤闪回两遍、找房本黄金翻多次）——原文只说一遍，正文复述凑字时必须报，同一信息只写一次。
17. 素材外事实漂移（对照原文报）：物件/数字被写到原文指定的位置/归属之外（录取通知书本在二楼卧室桌上，正文却写「一楼沙发上旧盒子里翻出录取通知书」；三千万本指父母留给陈迹的钱，正文却写「大哥手握三千万身家」）——把别的场景的素材挪用进来、或改挂归属时必须报。
...
_FIX_VERB = {
    ...
    "剧情事实": "删除素材里没有的人物身份/职业/背景/事件（如写陈迹当全职医生），改回素材设定（学生/精神病人）",
    ...
    "事实漂移": "把物件/数字改回素材指定的位置与归属（录取通知书只在二楼卧室桌、三千万=父母留给陈迹的钱），删除挪进本场景的素材外内容",
    "其它": "按 fix 修正",
}
...
def build_gap_feedback(generated: str, target: str, *, cap: int = 6) -> str | None:
    ...
    相对式（不列具体句）：逐句列"装饰间隙"会把原文本身的引语（如「王慧玲喜滋滋的
...
16. 禁止虚构素材外剧情事实：人物身份/职业/背景/关系/来历必须来自素材（原文/骨架/场景叶子）。素材没说陈迹是医生，就不得写「跑去医院当全职医生」「白大褂」这类脑补；素材说他是学生/精神病人就写学生/精神病人。只呈现素材已有的信息，不补人物来历。
17. 禁止动作流水账/平铺：原文一句带过的概括动作（如「陈硕与王慧玲在房间里翻翻找找，并将陈迹一家人的东西清理出来扔掉」）就照原文一句话带过；不得展开成「拉开抽屉→翻床底→摸床垫缝→开衣柜→扯衣服」逐微动作清单；不得「翻A→没找到→翻B→没找到」否定式反复。过程性动作一句话概括。
...
14. 禁止虚构素材外剧情事实：人物身份/职业/背景/关系/来历必须来自素材。素材没说陈迹是医生，就不得写「跑去医院当全职医生」「白大褂」这类脑补；只呈现素材已有的信息，不补人物来历。
...
陈迹沉默片刻，心中泛起一阵复杂的情绪，最终只是不自然地笑了笑。"""
...
15. 剧情事实虚构：出现了素材/上下文里没有的人物身份、职业、背景、事件（如素材说陈迹是学生，正文却写「跑去医院当全职医生」）——凭空脑补人物来历。对照素材/前后文判断。
...
20. 素材外事实漂移：物件/数字被写到素材指定的位置/归属之外（录取通知书本在二楼卧室桌，被搬到一楼旧盒子；三千万本指父母留给陈迹的钱，被安到大哥身家）——物件/数字必须待在素材给定的位置与归属。
