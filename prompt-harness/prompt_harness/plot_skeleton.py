...
2. 每个节拍 = 一个完整的场景动作：谁 + 做了什么具体动作 + 结果/反应。写清动作链，**不得用事件类型词（如「做评估」「问诊」「交谈」「对峙」「商议」）一笔带过**——必须写出该场景特有的具体内容：谁说了什么、反应如何。「两人处理遗像」要写成「陈硕把客厅里林川父母的黑白遗像合照扔进垃圾桶」
3. 按时间顺序组织，保留事件的因果关系
...
7. **标志性基调标注（防场景误读）**：每个场景开头（及基调切换处）在节拍前标注【基调：XX】，客观描述该场景的文学效果及其构成。若场景性质易被误读（如医生问问题被误读成普通看病），必须写出构成反差的具体内容 + 明示基调，不得用类型词一笔带过。示例：`【基调：黑色幽默（医生一本正经宣布五级评分 vs 林川对生死的淡定，反差制造荒诞）】医生老刘一本正经向林川宣布评估规则（按「无/很轻/中等/严重/非常严重」五级评分，可以吗）；林川同意后第一个问题就问他想不想结束生命——林川反问「结束谁的生命」，老刘答「你自己的」，林川答「那没有」`
8. 字数控制在原文的 25%-35%
9. 不输出任何解释性文字，只输出骨架本身

对照示例：
❌ 医生老刘对林川做精神评估（合并 + 抽象，错误）
✅ 医生老刘一本正经向林川宣布评估规则（按「无/很轻/中等/严重/非常严重」五级评分，可以吗）；林川同意后，第一个问题就问他想不想结束生命——林川反问「结束谁的生命」，老刘答「你自己的」，林川答「那没有」（保留开场内容与对话次序，正确）
