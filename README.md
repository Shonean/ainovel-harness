...
大多数 AI 写作工具的 prompt 是一个黑盒：大纲进、正文出，错了不知道该怪哪一层信息。AInovel Harness 把生成前的上下文拆成**五层信息阶梯**，再用**逐层充分性消融**回答一个平时没人量化的问题：
...
这个发现直接改不了工程路线：l1–l3 的价值被重新定位为**作者确认流**（human-in-the-loop 的审批节点），而非喂给模型的信息层。
...
标点/句式硬约束、对白间隙公式、防御性写作禁令——一千多行从上百万字生成实践中磨出来的中文 AI 腔检测与修正规则。这是生成后质量体系的领域级防线。
...
阶梯抽象适用于所有"不知道模型需要多细粒度才不出错"的长结构化生成，不只小说：剧本、技术文档、教材、游戏分支叙事都可以按同一套 l1→l5 思路换掉叶子实现。
